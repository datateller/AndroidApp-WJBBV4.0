...
public class DetailKnowledgeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        collect_button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!sdcardIsmount){
					DialogHelper.showDialog(DetailKnowledgeActivity.this, "无法收藏，请插入SD卡");
				    return;
				}
    ...
    //			TODO 需要修改icon的值，其值应该为icon的当前的目录
				...
				if(service.saveBasicKnowlegeToFile(path,filename,basicKnowledge)==false){
					DialogHelper.showDialog(DetailKnowledgeActivity.this, "收藏失败，您已经收藏该条知识");
			        return;	
				}
//				在collect目录下面保存string
				...
				DialogHelper.showDialog(DetailKnowledgeActivity.this, "恭喜您，您成功的收藏您喜欢的知识");
			}
		});
	}
	...
	private void getKnowledgeFromServerAndWriteToFile(final int knowledgeId,
		...
		final ProgressDialog myDialog = ProgressDialog.show(
				DetailKnowledgeActivity.this, "请稍等", "正在获取数据...", true, true);
				handler = new Handler() {
					@Override
					...
					public void handleMessage(Message msg) {
						...
						collectKnowledge=knowledge;
						if(filename==null) {
							DialogHelper.showDialog(DetailKnowledgeActivity.this, "获取知识失败");
							return;
						}
						InformationService service=new InformationService();
